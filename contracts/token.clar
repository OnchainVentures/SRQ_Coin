...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (ft-transfer? sarasotacoin amount from to)
  )
)
...
(define-read-only (get-balance (user principal))
  (ok (ft-get-balance sarasotacoin user))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply sarasotacoin))
)
...
(define-public (activate-token (coreContract principal) (stacksHeight uint))
  (let
    (
      (coreContractMap (try! (contract-call? .sarasotacoin-auth get-core-contract-info coreContract)))
    )
...
(define-data-var tokenUri (optional (string-utf8 256)) (some u"https://srqcoin.com"))
